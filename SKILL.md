...
name: jingqiu-DTC
description: >
  当用户要把 Amazon、Shopify、TikTok Shop、商品图、商品截图、ASIN、SKU brief、商品短描述或用户提供的参考广告，转成 SKU 级 DTC 商品创意故事版、品牌片视觉规则、5 格 16:9 横屏故事版图片、英文 15 秒或 30 秒 DTC 广告脚本时使用。必须先锁定 Product Truth Card，再创建 DTC Desire Angle，再输出 DTC Creative Reference Pack 和 Brand Film Look Pack，最后生成或记录故事版交付物、run.json、source_records.json 和本地验证结果。适用于 StoreClaw 商品内容工作流、Shopify PDP 视频、paid social 创意测试和定制商品故事版；Amazon listing demo 仅在用户明确要求时作为说明型例外路由。
---
...
2. 再创建 DTC Desire Angle，把购买情绪、身份、场景和品牌世界先定下来。
3. 再记录来源和合规状态。
4. 再拆解参考结构。
5. 再运行 DTC Creative Director。
6. 再创建 DTC Creative Reference Pack。
7. 再运行 Brand Film Look Director。
8. 再创建 Brand Film Look Pack。
9. 再让 Storyboard Design Expert 生成故事版和英文脚本。
10. 有本地文件权限时保存 `run.json`、`source_records.json`、`contact_sheet.png`、`panels/*.png`。
11. 当前环境不能生成或保存图片时，在 `run.json` 记录 `blocked_artifacts`，不能假装完成。
12. 最终回复保持三段输出，不默认暴露内部 JSON、完整 pack 或来源记录。
...
| 欲望角度 | `references/dtc-creative-pattern-library.md`、`references/dtc-scene-library.md` | 在 proof 之前先定义 target buyer、购买情绪、身份、场景和品牌世界 |
...
2. DTC Desire Angle
3. Live DTC Ad Reference Collection
4. Ad Reference Decomposition
5. DTC Creative Director
6. DTC Creative Reference Pack
7. Brand Film Look Director
8. Brand Film Look Pack
9. Storyboard Design Expert
10. Horizontal 5-Panel Storyboard Image Generation
11. English DTC Ad Script
12. Delivery Validation
```

### 1. Product Truth Lock

先创建 Product Truth Card。它是商品事实锁，负责防止 SKU 在后续创意、画面和脚本里漂移；视觉事实和产品功能事实都不能错。
...
- `product_function_lock`：商品到底做什么、怎么工作或怎么被使用、适用对象/场景、必须动作或安装条件、功能边界、不能说成什么功能。
...
在进入来源采集和创意导演前，先创建 `dtc_desire_angle`。它只定义购买欲望和品牌片方向，不能修改 Product Truth Card，也不能把未证实功能写成卖点。

必须记录这些字段：

- `target_buyer`：这个广告先打给谁，不写泛人群。
- `buying_emotion`：让买家先产生什么购买情绪，如想拥有、想送人、想升级、想更像某种生活状态。
- `desired_identity`：买家希望通过商品靠近的身份或自我感受。
- `use_occasion`：最有购买冲动的场景、季节、礼物时刻、日常仪式或内容语境。
- `visual_hook`：开头 2 秒先抓注意力的画面钩子，不先解释商品。
- `purchase_trigger`：从喜欢到想买的触发点，只能来自商品证据、可见场景或用户提供的品牌信息。
- `brand_world`：同一广告里的空间、光线、人物状态、色彩、道具和情绪世界。
- `what_to_feel_before_product_detail`：在详细解释商品前，观众应该先感到什么。
- `what_not_to_do`：tutorial tone、Amazon listing tone、feature checklist、step-by-step instruction。

完成标准：后续 `creative_type`、场景、镜头和脚本都必须先服务 `dtc_desire_angle`，再把功能证明嵌入自然动作。不能为了追求情绪而改变商品形状、颜色、材质、包装、产品功能、使用方式、工作机制、claim、评价、认证或平台 UI。
...
DTC Creative Director 决定“这个商品用什么 DTC 购买欲望被记住”。它不生成最终图片，不写最终脚本。功能证明只能作为购买欲望的支撑，不能把主线拉回 Amazon listing demo 或 instruction manual tone。

固定选择顺序：

```text
Step 1: 读取 Product Truth Card 和 DTC Desire Angle
Step 2: 选择 DTC-first creative_type
Step 3: 选择 scene_ref
Step 4: 选择 motion_chain
Step 5: 选择 model_ref
Step 6: 选择 camera_plan
Step 7: 选择 brand_film_mode
Step 8: 选择 script_pattern
Step 9: 选择 picture_fragment_logic
Step 10: 选择 end_frame
Step 11: 输出 DTC Creative Reference Pack
```
...
- `model_ref` 不能让人物抢商品。
- `camera_plan` 必须服务 hook、desire、proof、brand memory 或 CTA，不能只服务风格。
...
- 每个 panel 必须有购买情绪任务，不能只是 product proof。
...
2. product_function_lock
3. dtc_desire_angle
4. creative_type
5. selected_ad_reference logic
6. scene_ref
7. model_ref
8. motion_chain
9. camera_plan
10. brand_film_mode
11. brand_film_look_pack
12. brand_world_lock
13. cinematic_grammar
14. shot_function_map
15. material_and_light_rules
16. purchase_emotion_task per panel
17. picture_fragment_logic
18. end_frame
19. five-panel picture_fragment_logic
20. horizontal 16:9 contact sheet constraint
21. same shoot / same product / same lighting continuity
22. real physical contact shadows
23. no fake UI / no fake claims / no fake reviews
```
...
- `Visual Beat` 必须像 DTC ad / brand short 的镜头节奏，不像 Amazon listing demo。
- `Desire / Tension` 必须写购买情绪、场景身份或 occasion tension。
- `Product Role` 说明商品在这个情绪场景里承担什么购买触发任务。
- `On-screen Text` 和 `Voiceover` 使用自然英文，不写 how to use / shows how / demonstrates step 作为默认主线。
- `CTA Intent` 写购买意图，不默认写操作说明。
- `Camera / Motion` 可以进入内部 pack，不要在最终用户脚本里强制显得像制作说明。
- `Reference Logic` 不要默认出现在最终脚本里，除非用户明确要求内部拆解。
...
panel_aspect_ratio
contact_sheet
internal_camera_motion_pack
internal_reference_logic_pack
```
...
2. 没有 `product_function_lock`，不生成最终商品故事版。
3. 产品功能、使用方式、适用对象/场景、工作机制和功能边界不能错，不能被 DTC 欲望角度或品牌片语言重写。
4. 没有 DTC Desire Angle，不进入 DTC Creative Director。
5. 没有 DTC Creative Reference Pack，不生成故事版图片。
6. 没有 Brand Film Look Pack，不生成故事版图片。
7. 默认输出必须像 DTC ad / brand short，不像 Amazon listing demo。
8. 功能证明必须嵌入情绪场景，不能变成教程。
9. 每个 panel 必须有购买情绪任务，不只是 product proof。
10. 开头 2 秒必须先抓注意力，再解释商品。
11. 不要用 “how to use / shows how / demonstrates step” 作为默认主线。
12. Storyboard Design Expert 不能重新选择 `creative_type`。
13. 只能选择一个主 `creative_type`。
14. 不能选择商品无法自然出现的场景。
15. 不能选择无法形成 `start_state -> action -> end_state` 的动作。
16. 不能选择会隐藏商品或削弱商品证明的人物策略。
17. 镜头必须服务 hook、desire、proof、brand memory 或 CTA，不能只服务风格。
18. 不能固定第 3 格或第 5 格承担某个功能。
19. 每个 panel 的任务来自 `picture_fragment_logic`。
20. 每个 panel 都必须是 video-ready keyframe。
21. 英文脚本最终表不强制暴露 `Camera / Motion`；如生成内部 pack，必须匹配 `camera_selection` 和 `motion_selection`。
22. 最终用户输出不能暴露导演名、电影名或“像某导演”。
23. 不能把 prompt 当成交付物。
24. 不能默认硬爬平台或复制竞品素材。
25. 不能声称 validator 已经自动证明视觉商品一致性。
